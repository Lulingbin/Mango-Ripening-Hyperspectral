def save_13bands(csv_path, png_path):
    ...
    wavelengths = wave_df['Wavelength (nm)'].values  # 224

    # ----------------------------------------------------------
    # 1. 加载 FX10 数据（224 波段）+ 硬度值
    # ----------------------------------------------------------
    fx10 = pd.read_csv(csv_path)

    # 按硬度排序！！！
    fx10 = fx10.sort_values('Hardness').reset_index(drop=True)
    print(f"样品已按硬度排序: {fx10['Hardness'].min():.1f} -> {fx10['Hardness'].max():.1f}")

    # 获取硬度值用于着色
    hardness_values = fx10['Hardness'].values

    # 提取 224 波段光谱数据
    band_cols = [f'Band_{i}' for i in range(1, 225)]
    spectra = fx10[band_cols].values  # shape: (n_samples, 224)
    ...
    ax.set_ylabel('', fontsize=20 * n, labelpad=55)  # 16→20, pad 15→20
    ax.set_zlabel('Reflectance', fontsize=22 * n, labelpad=75, fontweight='bold')  # 18→22, pad 20→25
    ...
    fig.savefig(filepath, dpi=600, bbox_inches='tight', pad_inches=1.2)
...
# 运行部分保持不变
# ----------------------------------------------------------
raw_file = r'F:\study\manggo_program\Easypilpeline\new1.0\A_Dataset\FX10\Hardness_10_140\outlier_isolationforest\clean.csv'
raw_out = 'FX10_mean_spectra_13groups_trueWave_raw.png'

sg_file = r'F:\study\manggo_program\Easypilpeline\new1.0\A_Dataset\FX10\Hardness_10_140\outlier_isolationforest\Dataset\all\train_SG_all.csv'
sg_out = 'FX10_mean_spectra_13groups_trueWave_Sg.png'

snv_file = r'F:\study\manggo_program\Easypilpeline\new1.0\A_Dataset\FX10\Hardness_10_140\outlier_isolationforest\Dataset\all\train_SNV_all.csv'
snv_out = 'FX10_mean_spectra_13groups_trueWave_SNV.png'

zscore_file = r'F:\study\manggo_program\Easypilpeline\new1.0\A_Dataset\FX10\Hardness_10_140\outlier_isolationforest\Dataset\all\train_all.csv'
zscore_out = 'FX10_mean_spectra_13groups_z-score.png'
